...
pd.reset_option("all")
missing_more_than_50_percent_of_data = list(
    airbnb_data.columns[airbnb_data.isnull().mean() > 0.50]
)
for x in missing_more_than_50_percent_of_data:
    print(x)
airbnb_data = airbnb_data.drop(missing_more_than_50_percent_of_data, axis=1)
...
train_sqrt_mean_squared_error_pred = math.sqrt(
    mean_squared_error(y_train, rgf_y_train_pred)
)
test_sqrt_mean_squared_error_pred = math.sqrt(
    mean_squared_error(y_test, rgf_y_test_pred)
)
